...
def plot_pca(transformed_data, header, pca):
    ...
    scatter = sns.scatterplot(
        x="PC1", y="PC2", hue="Sample", data=pca_data, palette="viridis", s=60
    )
    ...
    for i, sample in enumerate(pca_data["Sample"]):
        if outliers_mask[i]:
            texts.append(
                plt.text(pca_data["PC1"][i], pca_data["PC2"][i], sample, fontsize=8)
            )
...
def perform_pca():
    if request.method == "POST":
        ...
        logging.info(
            f"PCA analysis completed successfully. Results saved to: {folder_path}"
        )
